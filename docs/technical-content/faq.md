...
2.  In the migrations folder, look for the one with the title ~"squash<sub>migrations</sub>"~; it should be near the top of migrations, as it should be one of the oldest in the migrations list. Copy the contents from this file into a scratch file.
...
8.  Create a new migration for clearing out the database table sequelize<sub>meta</sub> that tracks which of your migrations have run. Move the code from the scratch file generated in step 1 into a newly-generated migration (Migrations FAQ). Alter the number of migrations you are saving (i.e. those migrations that would occur after the schema load is run) by changing the value of const numberOfMigrationsToKeep.
...
Enjoy your newly clean database!
...
3.  Run this command to dump the data inside the DB: `pg_dump -d noipm-db-manager -U postgres -a -f /tmp/data.dump`
